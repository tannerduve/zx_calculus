...
/-!
# Structural Rules Tests

Tests for the structural rules of the ZX-calculus:
- Associativity of composition and tensor
- Identity laws
- Interchange law
-/
...
theorem comp_assoc : ZxEquiv ((H ; Z 1 1 1) ; X (1/2) 1 1) (H ; (Z 1 1 1 ; X (1/2) 1 1)) := by
  exact ZxEquiv.assoc_comp H (Z 1 1 1) (X (1/2) 1 1)

/-- Identity is left unit -/
theorem id_left_unit : ZxEquiv (id ; H) H := by
  exact ZxEquiv.unit_comp_l H

/-- Identity is right unit -/
theorem id_right_unit : ZxEquiv (H ; id) H := by
  exact ZxEquiv.unit_comp_r H

/-- Tensor with empty is left unit -/
theorem empty_tens_left : ZxEquiv (by simpa [Nat.zero_add] using (empty.tens H)) H := by
  exact ZxEquiv.unit_tens_l H

/-- Tensor with empty is right unit -/
theorem empty_tens_right : ZxEquiv (by simpa [Nat.add_zero] using (H.tens empty)) H := by
  exact ZxEquiv.unit_tens_r H

/-- Interchange law example -/
theorem interchange_example : ZxEquiv ((H.tens id) ; (id.tens H)) ((H ; id).tens (id ; H)) := by
